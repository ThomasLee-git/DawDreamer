...
//            std::cout << "lxd_process (" << start << ", " << end << ") " << myIsMessageBetween << " " << myMidiMessage.getDescription() << " " << myMidiMessagePosition << " " << int(myMidiMessagePosition - start) << std::endl;
...
PluginProcessor::loadMidi(const std::string& path)
{
    ...
    for (int t = 0; t < midiFile.getNumTracks(); t++) {
        ...
        for (int i = 0; i < track->getNumEvents(); i++) {
            ...
            std::cout << "track_" << t << " event_" << i << " " << m.getDescription() << std::endl;
            std::cout << "track_" << t << " event_" << i << " offset " << mySampleRate << " " << m.getTimeStamp() << std::endl;
